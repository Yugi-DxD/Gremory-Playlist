...
* **Decodificação Assíncrona:** O I/O de disco da cover art e a troca de imagens gigantes do WebGL não causam stuttering na live. O sistema utiliza a API nativa `createImageBitmap()`, que retira o trabalho pesado de decodificação de imagem da Main Thread do JavaScript, transferindo-o para worker threads em background. O overlay não congela o OBS enquanto carrega novos assets.
